...
新建独立分类管理模块 `MoAI.Classify`，作为**全站唯一分类管理来源**，运营者（admin+）统一维护四种分类：插件（`plugin`）、应用（`app`）、知识库（`kb`）、提示词（`prompt`）。前端按类型用 Tab 标签隔开，每类下可增删改分类项。
...
```
src/classify/
├── MoAI.Classify.Shared/  ClassifyTypes（常量：plugin/app/kb/prompt）
...
| 方法 | 路由 | 门禁 | 说明 |
|---|---|---|---|
| GET | `/classify/list?type=plugin\|app\|kb\|prompt\|skill` | admin | 按类型返回分类列表 `{items:[{classifyId,name,description,emoji}]}` |
...
1. **类型常量**：`ClassifyTypes.Plugin="plugin"`、`App="app"`、`Kb="kb"`、`Prompt="prompt"`、`Skill="skill"`。复用现有 `ClassifyEntity.Type`（≤10 字符）存字符串。
...
6. **前端 Tab**：`Page` + `Tabs`（plugin/app/kb/prompt/skill），每 Tab 一张分类列表；增删改走弹窗（名称 + 描述 + 表情），危险删除用 `Popconfirm`；文案走 i18n（zh-CN + en-US）。
...
- 应用（`app`）、知识库（`kb`）与提示词（`prompt`）业务层尚未实现（提示词功能开发中），本模块只提供分类字典维护；资源引用校验暂以插件为准，`app/kb/prompt` 分类删除暂不校验引用（预留扩展点）。
