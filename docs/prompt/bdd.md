...
- Then 默认显示市场 Tab，加载已上架提示词并渲染分类列表（全部/各分类，含 emoji）、卡片与分页
- When 切换到「我的提示词」Tab
...
- Then 市场卡片可查看详情（Markdown 渲染、横线分隔且无背景色/边框容器）并复制内容；我的提示词卡片可编辑/申请上架/撤回/删除；编辑器左侧 Markdown 编辑、右侧实时预览，分类选项显示 emoji；团队分区成员只读、管理员可创建/编辑/删除/申请上架
