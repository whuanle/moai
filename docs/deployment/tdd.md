...
| @DEP-S3、@DEP-S6 | `deploy-compose.sh` 由 `.env` 生成 `Storage` 五项（含 `S3_ENDPOINT` 自定义域名，2026-09-23 起）；未配 S3 时 S3 客户端 500（已实证）；.env.example 走查 | 走查一致（2026-09-23 复核） |
...
| @DEP-S15 | `deploy-compose.sh` 生成的 `configs/system.json` 中 Server/Storage.Endpoint 取自 `.env`（含 `S3_ENDPOINT`）；容器内文件一致；缺失时 entrypoint 报错退出 | 待执行（2026-09-23 新增） |
...
3. ✅ D3 已修复：2026-09-03 由 entrypoint 生成 S3 段；**2026-09-23 改为 `deploy-compose.sh` 由 `.env` 生成 `Storage` 五项**，容器内存储 e2e 7/7（[@DEP-S3](./bdd.md#dep-s3)）。遗留注意：`Storage.Endpoint` 单端点设计（须同时被应用与上传客户端可达），见 [docker.md 3.1](./docker.md) 与 sdd 缺陷表。
