...
- **维护模型**：图谱详情「模型」页增删改实体类型、关系类型；关系类型可选起止实体类型约束。删除仍有节点 / 边或被关系类型引用的类型会被拒绝（409）。
...
- **接入外部图谱**：新建 →「接入已有」→ 填名称与目标 database（外部实例的库名；Memgraph 社区版单库，填默认库）。接入后只读内省标签 / 关系类型 / 属性键。
...
| 接入图新增节点 / 类型返回 409 | 设计如此：接入图只读；画布亦不支持接入图 |
...
1. `dotnet build src/MoAI/MoAI.csproj` → 0 错误；`dotnet test tests/MoAI.KnowledgeGraph.Tests/` → 39/39。
2. 后端运行且 Memgraph 可达、`KG_ENABLED=true` 后执行 `node local-dev/kg-e2e.mjs http://127.0.0.1:5000` → 覆盖 [@KG-S1](../knowledgegraph/bdd.md#kg-s1)…[@KG-S16](../knowledgegraph/bdd.md#kg-s16)（S15 成员只读由单测覆盖）；无图数据库时脚本 SKIP。
3. 浏览器走查：`/knowledge-graph` 与团队「知识图谱」分区建托管图（含模板）→ 图览过滤 / 搜索 / 展开 → 模型 / 节点 / 边维护（Member 只读）→ 删除清库；接入外部库 → 只读内省 → 删除仅移除登记；重启 Memgraph 容器验证快照恢复。
