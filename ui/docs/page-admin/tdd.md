...
> 本文只做「场景 → 验证物 → 结果」映射。Settings 页有组件单测，OauthConnect 仍为手工走查；后端接口自动化见上游 TDD。
...
| @FE-PG-S7 ~ @FE-PG-S9（Settings） | `ui/src/pages/settings/__tests__/Settings.test.tsx`（root 加载回显、开关+连接项保存、非 root 重定向；4/4） | PASS（2026-09-10） |
...
| 两页编译与全量回归 | `cd ui && npm run test` + `npm run typecheck` | PASS（历史 42/42、0 错误 2026-09-01；2026-09-10 含 Settings 4/4） |
...
- Settings 页已补组件单测（@FE-PG-S7~S9）；OauthConnect 仍零组件自动化（表单校验、Modal 交互走查）；补测试的标准流程见 [../dashboard-testing/sop.md](../dashboard-testing/sop.md) 第 2 节。
