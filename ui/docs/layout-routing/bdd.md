...
Feature: 二级页面隐藏一级侧边栏
  @FE-LR-S22 @manual
  Scenario: 进入团队详情及下属二级页面时隐藏一级侧边栏
    Given 已登录用户
    When 进入 /team/:id 及其下属任意页面（团队分区/内部应用/应用设计/应用对话/知识库/知识图谱/团队提示词编辑）
    Then 一级侧边栏整体不渲染（非收缩），内容区占满全宽
    And 页面自身二级导航（团队分区菜单/面包屑/返回按钮）保持可用
