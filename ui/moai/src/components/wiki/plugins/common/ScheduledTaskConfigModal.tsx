...
interface ScheduledTaskConfigModalProps {
  open: boolean;
  onCancel: () => void;
  onConfirm: (
    cron: string,
 ...
 * 包含 Cron 配置和自动处理配置
 */
export default function ScheduledTaskConfigModal({
  open,
  onCancel,
  onConfirm,
  wikiId,
  loading = false,
}: ScheduledTaskConfigModalProps) {
  ...
  const handleConfirm = async () => {
    try {
      // 验证 cron 表达式
      await form.validateFields(["cron"]);
      const values = form.getFieldsValue();

      // 如果不启用自动处理，提交默认配置
      if (!isAutoProcess) {
        const defaultConfig: WikiPluginAutoProcessConfig = {
          partion: {
          ...
          aiPartion: null,
          preprocessStrategyType: null,
          preprocessStrategyAiModel: null,
          isEmbedding: false,
          isEmbedSourceText: false,
          threadCount: null,
        };
        onConfirm(values.cron, false, defaultConfig);
        handleCancel();
        return;
      }

      // 验证自动处理相应的字段
      const fieldsToValidate: string[] = ["cron"];
      ...
      onConfirm(values.cron, true, autoProcessConfig);
      handleCancel();
    } catch (error) {
    ...
    <>
      ...
      >
        <Form form={form} layout="vertical">
          <Divider orientation="left">定时配置</Divider>
          ...
          <Divider orientation="left">自动处理配置</Divider>
