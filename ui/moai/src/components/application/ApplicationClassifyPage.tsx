...
import {
  AppClassifyItem,
  CreateAppClassifyCommand,
  UpdateAppClassifyCommand,
  DeleteAppClassifyCommand,
} from "../../apiClient/models";
import { proxyFormRequestError, proxyRequestError } from "../../helper/RequestError";

export default function ApplicationClassifyPage() {
  const navigate = useNavigate();
  const [classList, setClassList] = useState<AppClassifyItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [addModalVisible, setAddModalVisible] = useState(false);
  const [editModalVisible, setEditModalVisible] = useState(false);
  const [currentClass, setCurrentClass] = useState<AppClassifyItem | null>(null);
  ...
  // 打开编辑弹窗
  const handleEditClick = (item: AppClassifyItem) => {
  ...
  const handleAddSubmit = async (values: { name: string; description: string }) => {
    ...
    try {
      const client = GetApiClient();
      const requestBody: CreateAppClassifyCommand = {
        name: values.name,
        description: values.description,
      };
      await client.api.admin.appclassify.add_classify.post(requestBody);
  ...
  const handleEditSubmit = async (values: { classifyId: number; name: string; description: string }) => {
    ...
    try {
      const client = GetApiClient();
      const requestBody: UpdateAppClassifyCommand = {
        classifyId: values.classifyId,
        name: values.name,
        description: values.description,
      };
      await client.api.admin.appclassify.update_classify.post(requestBody);
  ...
  // 删除分类
  const handleDelete = async (item: AppClassifyItem) => {
    try {
      const client = GetApiClient();
      const requestBody: DeleteAppClassifyCommand = {
        classifyId: item.classifyId!,
      };
      await client.api.admin.appclassify.delete_classify.post(requestBody);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "action",
      width: 150,
      render: (_: unknown, record: AppClassifyItem) => (
        <Space size="small">
    ...
    <div className="page-container">
      ...
      <div className="moai-page-header">
        <div>
          <Space align="center">
            ...
            <h1 className="moai-page-title moai-page-title-inline">应用分类管理</h1>
          </Space>
          <p className="moai-page-subtitle">管理和组织你的应用分类</p>
        </div>
      </div>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleAddSubmit}>
          ...
          <Form.Item style={{ marginBottom: 0, marginTop: 24 }} className="form-item-footer">
            <Space>
