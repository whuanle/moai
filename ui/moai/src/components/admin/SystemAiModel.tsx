...
export default function SystemAIModel() {
  ...
  const fetchProviders = async () => {
    ...
    try {
      ...
      const response = await client.api.aimodel.system_providerlist.get();
  ...
  const fetchModels = async (provider: string, index: number) => {
    ...
    try {
      ...
      const response = await client.api.aimodel.type.system_modellist.post(requestBody);
  ...
  const handleEditClick = (model: AiNotKeyEndpoint, provider: string) => {
    ...
    editForm.setFieldsValue({
      ...
      isSystem: (model as any).isSystem ?? true,
