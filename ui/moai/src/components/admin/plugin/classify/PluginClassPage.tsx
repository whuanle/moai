...
import {
  PluginClassifyItem,
  CreatePluginClassifyCommand,
  UpdatePluginClassifyCommand,
  DeletePluginClassifyCommand,
...
export default function PluginClassPage() {
  ...
  const [currentClass, setCurrentClass] = useState<PluginClassifyItem | null>(null);
  ...
  const fetchClassList = async () => {
    ...
    try {
      ...
      const response = await client.api.admin.classify.classify_list.get();
  ...
  // 处理编辑按钮点击
  const handleEditClick = (item: PluginClassifyItem) => {
  ...
  // 处理删除按钮点击
  const handleDeleteClick = async (item: PluginClassifyItem) => {
    try {
      const client = GetApiClient();
      const requestBody: DeletePluginClassifyCommand = {
        classifyId: item.classifyId!,
      };
      await client.api.admin.classify.delete_classify.post(requestBody);
  ...
  // 处理新增分类提交
  const handleAddSubmit = async (values: CreatePluginClassifyCommand) => {
    setSubmitting(true);
    try {
      const client = GetApiClient();
      await client.api.admin.classify.add_classify.post(values);
  ...
  const handleEditSubmit = async (values: any) => {
    ...
    try {
      ...
      await client.api.admin.classify.update_classify.delete(requestBody);
  ...
  const classColumns = [
    ...
    },
    {
      ...
      key: "action",
      width: 150,
      render: (_: any, record: PluginClassifyItem) => (
        <Space size="small">
          <Button
            type="link"
            size="small"
