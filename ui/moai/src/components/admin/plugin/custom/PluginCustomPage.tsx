...
import {
  ...
  QueryTeamCustomPluginFunctionsListCommand,
...
export default function PluginCustomPage() {
      ...
      if (response?.items) {
        ...
        if (filterClassifyId !== undefined) {
          filteredItems = filteredItems.filter(
            (item) => item.classifyId === filterClassifyId
          );
        }
  ...
  }, [messageApi, searchName, filterType, filterClassifyId]);
  ...
  useEffect(() => {
    ...
    fetchClassifyList();
    fetchPluginList();
  }, [fetchCurrentUser, fetchClassifyList, fetchPluginList]);
  ...
  const handleViewFunctions = useCallback(
    async (record: PluginBaseInfoItem) => {
      ...
      try {
        const client = GetApiClient();
        const requestData: QueryTeamCustomPluginFunctionsListCommand = {
          pluginId: record.pluginId,
        };
  ...
  const columns = useMemo(
    ...
    [currentUser?.userId, handleViewFunctions, handleEdit, handleDelete, classifyList]
  );
              ...
              <Col span={12}>
                <Form.Item
                  ...
                  rules={[
                    ...
                    { pattern: /^[a-zA-Z_]+$/, message: "插件名称只能包含字母下划线" },
                  ]}
                >
