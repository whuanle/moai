...
import { AIChatModelCard, AIEmbeddingModelCard, AIText2ImageModelCard, AITTSModelCard, AISTTModelCard, AIRealtimeModelCard } from '../../lobechat/types/aiModel';
...
export default function AIModel() {
  ...
  const fetchProviders = async () => {
    ...
    try {
      ...
      const response = await client.api.aimodel.providerlist.get();
      if (response?.providers) {
        setProviders(response.providers.map(provider => ({
          ...provider,
          models: [],
          loading: false
        })));
  ...
  const fetchModels = async (provider: string, index: number) => {
    ...
    try {
      ...
      const response = await client.api.aimodel.type.modellist.post(requestBody);
