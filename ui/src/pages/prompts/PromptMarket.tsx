...
/** 提示词市场：展示所有已上架（is_public）的提示词，所有登录用户可浏览与复制内容 */
export function PromptMarket() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setItems(await getPromptMarketList({ keywords, promptClassId }))
    } catch {
      // 错误已由全局请求中间件统一提示
    } finally {
      setLoading(false)
    }
  }, [keywords, promptClassId])
  ...
  const columns: TableColumnsType<PromptItem> = useMemo(
    () => [
      ...
      {
        title: t('prompt.colCounter'),
        key: 'counter',
        width: 110,
        render: (_, record) => (
          <Text type="secondary">
            <FireOutlined /> {record.counter ?? 0}
          </Text>
        ),
      },
      { title: t('prompt.colCreator'), dataIndex: 'createUserName', width: 120, render: (v: string | null) => v || '-' },
      {
  ...
  return (
    <Page>
      ...
      <DataTable<PromptItem>
        ...
        toolbar={
          <Space size={12}>
            <Text type="secondary">{t('prompt.marketHint')}</Text>
