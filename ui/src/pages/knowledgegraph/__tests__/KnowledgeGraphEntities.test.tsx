...
function renderEntities(graphEnabled = true, myRole: number | null = 1) {
  return render(<MemoryRouter><KnowledgeGraphEntities graphId={1} teamId={1} graphEnabled={graphEnabled} myRole={myRole} /></MemoryRouter>)
}
...
describe('KnowledgeGraphEntities', () => {
  ...
  it('渲染节点行并映射实体类型名称', async () => {
    ...
    expect(getKnowledgeGraphNodes).toHaveBeenCalledWith(1, { entityTypeId: null, keyword: '', pageNo: 1, pageSize: 20 })
  })

  it('无实体类型时「新建实例」禁用', async () => {
    vi.mocked(getKnowledgeGraphSchema).mockResolvedValue({ entityTypes: [], relationTypes: [] })
    renderEntities()
    const button = await screen.findByRole('button', { name: /新建实例/ })
    expect(button).toBeDisabled()
  })

  it('有实体类型时「新建实例」可用', async () => {
    renderEntities()
    const button = await screen.findByRole('button', { name: /新建实例/ })
    expect(button).toBeEnabled()
  })

  it('能力未开启时不显示「新建实例」入口', async () => {
