...
internal sealed partial class MistralFunction
{
    ...
    private static void ValidFunctionName(string name)
    {
        ...
        if (name.Length > 64)
        {
            throw new ArgumentException("The name of the function must be less than or equal to 64 characters.", nameof(name));
        }
    ...
    private static KernelJsonSchema GetDefaultSchemaForTypelessParameter(string? description)
    {
        ...
        {
            return KernelJsonSchema.Parse($@"{{""type"":""string"",""description"":""{description}""}}");
        }
