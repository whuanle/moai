...
public class QueryWikiDocumentsCommandHandler : IRequestHandler<QueryWikiDocumentsCommand, QueryWikiDocumentsCommandResponse>
{
    ...
    public QueryWikiDocumentsCommandHandler(DatabaseContext databaseContext, ITeamService teamService, IUserContextProvider userContextProvider)
    {
    ...
    public async Task<QueryWikiDocumentsCommandResponse> Handle(QueryWikiDocumentsCommand request, CancellationToken cancellationToken)
    {
        ...
        var items = await _databaseContext.WikiDocuments
            .Where(x => x.WikiId == request.WikiId)
            .OrderBy(x => x.Id)
            .Select(x => new WikiDocumentItem
            {
                DocumentId = x.Id,
                WikiId = x.WikiId,
                Title = x.Title,
