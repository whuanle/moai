...
/// <summary>
/// <inheritdoc cref="DeleteWikiCrawlerConfigCommand"/>
/// </summary>
public class DeleteWikiCrawlerConfigCommandHandler : IRequestHandler<DeleteWikiCrawlerConfigCommand, EmptyCommandResponse>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DeleteWikiCrawlerConfigCommandHandler"/> class.
    /// </summary>
    /// <param name="databaseContext"></param>
    /// <param name="systemOptions"></param>
    /// <param name="mediator"></param>
    /// <param name="serviceProvider"></param>
    public DeleteWikiCrawlerConfigCommandHandler(DatabaseContext databaseContext, SystemOptions systemOptions, IMediator mediator, IServiceProvider serviceProvider)
    {
    ...
    /// <inheritdoc/>
    public async Task<EmptyCommandResponse> Handle(DeleteWikiCrawlerConfigCommand request, CancellationToken cancellationToken)
    {
        ...
        // 删除配置
        await _databaseContext.SoftDeleteAsync(_databaseContext.WikiPluginCrawlerPages.Where(x => x.WikiId == request.WikiId && x.ConfigId == request.ConfigId));
        await _databaseContext.SoftDeleteAsync(_databaseContext.WikiPluginConfigs.Where(x => x.Id == request.ConfigId));
        await _databaseContext.SoftDeleteAsync(_databaseContext.WorkerTasks.Where(x => x.BindType == "wiki_crawler" && x.BindId == request.ConfigId));
    ...
    private async Task DeleteDocumentsAsync(DeleteWikiCrawlerConfigCommand request, CancellationToken cancellationToken)
    {
        var webDocumentIds = await _databaseContext.WikiPluginCrawlerPages.Where(x => x.WikiId == request.WikiId).Select(x => x.WikiDocumentId)
            .ToArrayAsync();
