...
public interface IVariableService
{
    ...
    /// <summary>
    /// 将一组键值对中的 Value 做 <c>{key}</c> 团队变量替换（私密变量解密），Key 不变；未匹配到变量的占位符保留原文.
    /// </summary>
    /// <param name="teamId">团队 id.</param>
    /// <param name="values">待替换的键值对集合.</param>
    /// <param name="cancellationToken">取消令牌.</param>
    /// <returns>返回替换后的键值对集合（新集合，不修改入参）.</returns>
    Task<IReadOnlyList<KeyValueString>> SubstituteAsync(long teamId, IReadOnlyCollection<KeyValueString> values, CancellationToken cancellationToken = default);
}
