...
public class S3Storage : IStorage, IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="S3Storage"/> class.
    /// </summary>
    /// <param name="systemOptions"></param>
    public S3Storage(SystemOptions systemOptions)
    {
    ...
    public async Task<string> GeneratePreSignedUploadUrlAsync(FileObject fileObject)
    {
        ...
        // todo: 可以在对象 metadata 中添加最大的文件大小信息
    ...
    public async Task<IReadOnlyDictionary<string, Uri>> GetFilesUrlAsync(IReadOnlyCollection<KeyValueString> objectKeys, TimeSpan expiryDuration)
    {
        IEnumerable<Task<KeyValuePair<string, Uri>>>? tasks = objectKeys.Select(async key =>
        {
            ...
            string? url = await _s3Client.GetPreSignedURLAsync(request);
            return new KeyValuePair<string, Uri>(key.Key, new Uri(url));
        });
