...
internal partial class WikiDocumentChunkEmbeddingConfiguration : IEntityTypeConfiguration<WikiDocumentChunkEmbeddingEntity>
{
    ...
    {
        var entity = builder;
        entity.HasKey(e => e.Id).HasName("idx_wiki_document_chunk_embedding_primary");

        entity.ToTable("wiki_document_chunk_embedding", tb => tb.HasComment("切片向量化内容"));

        entity.HasIndex(e => new { e.DocumentId, e.MetadataType }, "idx_wiki_document_chunk_embedding_doc_deriv");

        entity.HasIndex(e => e.Id, "idx_wiki_document_chunk_embedding_slice_deriv");

        entity.HasIndex(e => e.MetadataType, "idx_wiki_document_chunk_embeddingx_deriv_type");
