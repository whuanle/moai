...
/// <summary>
/// <inheritdoc cref="CreateExternalRelationTypeCommand"/>
/// </summary>
public class CreateExternalRelationTypeCommandHandler : IRequestHandler<CreateExternalRelationTypeCommand, SimpleLong>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CreateExternalRelationTypeCommandHandler"/> class.
    /// </summary>
    /// <param name="databaseContext">数据库上下文.</param>
    /// <param name="externalAuthorizer">外部授权器.</param>
    /// <param name="settingsService">知识图谱设置.</param>
    public CreateExternalRelationTypeCommandHandler(DatabaseContext databaseContext, IExternalKnowledgeGraphAuthorizer externalAuthorizer, IKnowledgeGraphSettingsService settingsService)
    {
    ...
    /// <inheritdoc/>
    public async Task<SimpleLong> Handle(CreateExternalRelationTypeCommand request, CancellationToken cancellationToken)
    {
        ...
        await EnsureEntityTypesExistAsync(request.KnowledgeGraphId, request.SourceTypeId, request.TargetTypeId, cancellationToken);
    ...
    private async Task EnsureEntityTypesExistAsync(long KnowledgeGraphId, long? sourceTypeId, long? targetTypeId, CancellationToken cancellationToken)
    {
        if (sourceTypeId.HasValue)
        {
            var sourceExist = await _databaseContext.KnowledgeGraphEntityTypes
                .AnyAsync(x => x.KnowledgeGraphId == KnowledgeGraphId && x.Id == sourceTypeId.Value, cancellationToken);
            if (!sourceExist)
            {
        ...
        if (targetTypeId.HasValue)
        {
            var targetExist = await _databaseContext.KnowledgeGraphEntityTypes
                .AnyAsync(x => x.KnowledgeGraphId == KnowledgeGraphId && x.Id == targetTypeId.Value, cancellationToken);
            if (!targetExist)
            {
                throw new BusinessException("关联的实体类型不存在.") { StatusCode = 400 };
            }
